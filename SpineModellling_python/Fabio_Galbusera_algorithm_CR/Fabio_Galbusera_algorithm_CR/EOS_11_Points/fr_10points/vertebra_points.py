...
class vertebra_points():
    def __init__(self):
        self.name = ''
        self.points = np.zeros((13, 3), dtype=float)

    ## edited D.L (added Spinous_Process)
    def get_point_id(self, pname): # Edited D.L (added 'Spinous_Process')
    ...
    def get_crop_region(self, factor, size_x, size_y, size_z, padding):
        ...
        if (self.name != 'Hips') and (self.name != 'S1'):
            limit_low = 0
            limit_up = 10 # Edited D.L (from 9 to 10)
        if self.name == 'Hips':
            limit_low = 11 # Edited D.L (from 10 to 11)
            limit_up = 12 # Edited D.L (from 11 to 12)
        if self.name == 'S1':
            limit_low = 2
            limit_up = 3
